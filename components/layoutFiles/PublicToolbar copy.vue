...
<template>
    <Disclosure as="nav" class="bg-white fixed w-[100%] z-10" v-slot="{ open }">
        <div class="mx-auto md:max-w-7xl justify-center w-[100%] px-2 sm:px-6 lg:px-8">
            
            <div class="relative flex h-20 items-center justify-between">

                <div class="md:flex md:flex-1 md:items-center md:justify-between sm:justify-start">

                    <div class="flex-shrink-0 cursor-pointer flex items-center">
                        <RouterLink to="/" class="flex md:items-center">
                        ...
                        <div
                            ...
                            <span v-for="(nav, i) in navigation" :key="i">
                                <NuxtLink :to="nav.href"
                                    :class="nav.current ? 'flex text-gray-600 cursor-pointer transition-colors duration-300 font-semibold text-blue-600' : 'flex text-gray-600 hover:text-[#0065f2] cursor-pointer transition-colors duration-300'">
                                    {{ nav.name }}
                                </NuxtLink>
                            </span>
